...
// ─── Priority helpers ─────────────────────────────────────────────────────────
function getPriority(dueDate?: string | null): "high" | "medium" | "low" {
...
async function fetchTasks(eventID: number): Promise<Task[]> {
...
async function addCustomTaskAPI(task: Omit<Task, "taskID">): Promise<Task> {
...
const TasksPage = ({ event, selectedVendors, onBack, onFinish }: Props) => {
  ...
  const toggle = useCallback(async (task: Task) => {
    ...
    setTasks((p) =>
      p.map((t) =>
        t.taskID === task.taskID ? { ...t, isCompleted: next } : t,
      ),
    );
    try {
                        ...
                        <div className="task-body">
                          ...
                          {dueLabel && (
                            <div
                              ...
                              style={{
                                color:
                                  priority === "high" ? "#e07070" : undefined,
                              }}
                            >
                              {dueLabel}
                            </div>
...
// ## סיכום הזרימה המלאה:
// ```
// בחירת ספק בעמוד ספקים
//         ↓
//   toggleVendor()
//         ↓
// insertTasksForVendor() ← POST /api/tasks/generate
//         ↓
//   הפרוצדורה בודקת: כבר יש משימות לספק הזה?
//         ↓              ↓
//       כן → skip    לא → INSERT מ-TaskTemplates
//         ↓
// "המשך למשימות" ← saveSelectedVendors()
//         ↓
// TasksPage טוען הכל עם GET /api/tasks?eventId=X
